...
def load_features_boxes(cfg,split):
    ...
    if cfg.TRAIN.DATASET == 'Phakirtubes' or  cfg.TRAIN.DATASET == 'Phakirtubesmatis':
        features = {feat['file_name'] : {'features':feat['features'], 'mask':feat['mask']} for feat in features}

        for file,boxes in features.items():
            #TODO: HARD-CODED 8!
            assert all(len(feats)==8 for feats in boxes['features'].values()), f'Incorrect feature length in image{file}. Excpected size {cfg.FEATURES.DIM_FEATURES}'
    else:
