...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        for idx, task in enumerate(self.tasks):
            if self.regions and task in self._region_tasks:
                if self.features:
                    extra_head = head_helper.TransformerRoIHeadTrans(
                                cfg,
                ...
                extra_head = head_helper. MultiSequenceTransformerBasicHead(
                            self.cfg,
                            self.embed_dim,
                            self.num_classes[idx],
                            dropout_rate=cfg.MODEL.DROPOUT_RATE,
    ...
    def forward(self, x, features=None, boxes_mask=None, rpn_masks=None):
        # breakpoint()
        out = {}
        x = x[0]
        ...
        for task in self.tasks:
            ...
            out[task] = extra_head(x, features, boxes_mask, rpn_masks)
...
class MMViT(MViT):
    """
    MultiMultiscale Vision Transformers
    Haoqi Fan, Bo Xiong, Karttikeya Mangalam, Yanghao Li, Zhicheng Yan, Jitendra Malik, Christoph Feichtenhofer
    https://arxiv.org/abs/2104.11227
    """
    ...
    def __init__(self, cfg):
        ...
        for idx, task in enumerate(self.tasks):
            if self.regions and task in self._region_tasks:
                ...
                if self.recogn and task in self.recog_tasks:
                    recog_head = head_helper.MultiSequenceTransformerBasicHead(
                                    cfg,
                                    self.embed_dim,
                                    self.num_classes[idx],
                                    dropout_rate=cfg.MODEL.DROPOUT_RATE,
    ...
    def forward(self, x_seq, features=None, boxes_mask=None, image_names=None):
        # breakpoint()
        out = {}
        outs = []
        for x in x_seq:
        ...
        for task in self.tasks:
            extra_head = getattr(self, "extra_heads_{}".format(task))
            out[task] = extra_head(outs, features, boxes_mask, image_names)
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, n_heads):
        ...
        self.enc_self_attn = torch.nn.MultiheadAttention(d_model, n_heads, batch_first=True)
    ...
    def forward(self, enc_inputs, mask=None):
        ...
        attn_outputs, attn_scores = self.enc_self_attn(enc_inputs, enc_inputs, enc_inputs, key_padding_mask=mask)
...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, n_heads):
        ...
        self.dec_enc_attn = torch.nn.MultiheadAttention(d_model, n_heads, batch_first=True)
        self.pos_ffn = PoswiseFeedForwardNet(d_model, 256*4)
    ...
    def forward(self, dec_inputs, enc_outputs, mask=None):
        ...
        dec_outputs, _ = self.self_attn(dec_inputs, dec_inputs, dec_inputs, key_padding_mask=mask)
        dec_outputs = self.layernorm1(dec_outputs + dec_inputs)  # Residual Connection

        # Encoder-Decoder Attention with Residual Connection
        dec_outputs1, dec_enc_attn = self.dec_enc_attn(dec_outputs, enc_outputs, enc_outputs, key_padding_mask=mask)
...
class VideoTransformerPerFrame(nn.Module):
    ...
    def generate_triangular_mask(self, x):
        _, video_lenght, _ = x.shape
        src_mask = torch.triu(torch.ones(video_lenght + 1, video_lenght) * float('-inf'))[1:, :].unsqueeze(0).cuda()
